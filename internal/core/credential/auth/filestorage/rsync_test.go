...
func handleFakeRsync(c net.Conn, acceptUser, acceptPass string) {
	...
	resp := make([]byte, 16)
	if _, err := readFullRSTest(c, resp); err != nil {
		return
	}
	// Verify: MD5(challenge || password), XOR'd 16 times.
	// / 验证：MD5(challenge || password)，XOR 16 次。
	expected := rsyncHashTest(acceptPass, challenge)
	for i := 0; i < 16; i++ {
...
func readFullRSTest(c net.Conn, buf []byte) (int, error) {
	total := 0
	for total < len(buf) {
