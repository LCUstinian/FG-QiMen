...
// Authenticate implements credential.Authenticator. Tries each community
// string as the password (user is empty for SNMPv2c). / Authenticate
// 实现 credential.Authenticator。按顺序尝试每个 community string（user 为空）。
func (a *SNMPAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []credential.Cred, timeout time.Duration) (*credential.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
func init() {
	credential.Register(NewSNMPAuthenticator())
}
