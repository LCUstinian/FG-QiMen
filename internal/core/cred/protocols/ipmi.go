...
// order. / Authenticate 实现 cred.Authenticator。按顺序尝试每个
// cred。
func (a *IPMIAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
func init() {
	cred.Register(NewIPMIAuthenticator())
}
