...
// Authenticate implements cred.Authenticator. Tries each cred in order;
// returns the first hit or nil.
//
// Authenticate 实现 cred.Authenticator。按顺序尝试每个 cred；首个命中
// 返回 Hit。
func (a *ElasticsearchAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
func init() {
	cred.Register(NewElasticsearchAuthenticator())
}
