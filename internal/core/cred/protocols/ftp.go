...
// 返回，否则返回 nil。
func (a *FTPAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
// ——立即关闭。
func ftpTry(ctx context.Context, addr string, c cred.Cred, timeout time.Duration) (bool, error) {
