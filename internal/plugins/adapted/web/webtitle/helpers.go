// helpers.go — HTTP client setup, redirect handling, header
// formatting, title extraction, favicon hash fetch, and the small
// display helpers for the webtitle plugin. Extracted from
// webtitle.go (was one 378-line file) so the high-level Identify
// flow stays scannable.
//
// helpers.go — webtitle 插件的 HTTP 客户端搭建、重定向处理、头
// 格式化、标题抽取、favicon 哈希计算与小展示 helper。从
// webtitle.go（原 378 行）抽出；让 webtitle.go 的高层 Identify 流程
// 更易读。
package webtitle
...
// (extractTitle, below) so the next person who wants to extend the
// match can find them without grepping the whole package.
//
// extractTitle 的正则。与唯一调用点放一起，方便后续扩展。
var (
