...
凭据测试覆盖 **26 个服务**（SSH + FTP + MySQL + Redis + Memcached + MongoDB +
MSSQL + SMB + PostgreSQL + Elasticsearch + VNC + Telnet + Oracle + WinRM +
POP3 + IMAP + SOCKS5 + LDAP + SNMPv2c + Rsync + Docker + RabbitMQ + Modbus +
IPMI v2.0 + BACnet + NFS），均强制不做漏洞利用（`creds.txt` 是唯一副作用）。
