...
// cred.Authenticator。
func (a *SOCKS5Authenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
func init() {
	cred.Register(NewSOCKS5Authenticator())
}
