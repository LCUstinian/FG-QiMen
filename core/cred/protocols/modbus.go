...
// Authenticate implements cred.Authenticator. Tries each cred;
...
// keep the contract uniform. / Authenticate 实现 cred.Authenticator。
// 按顺序尝试每个 cred；Modbus TCP 本身无凭据所以忽略 cred.User /
// cred.Pass——只探针。仍迭代 creds 保持契约统一。
func (a *ModbusAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
func init() {
	cred.Register(NewModbusAuthenticator())
}
