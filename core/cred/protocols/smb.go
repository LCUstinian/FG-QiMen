...
//
// Authenticate 实现 cred.Authenticator。按顺序尝试每个 cred；首个
// 命中返回，否则返回 nil。
//
...
func (a *SMBAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
