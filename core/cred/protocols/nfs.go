...
// cred.Authenticator。
func (a *NFSAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
func init() {
	cred.Register(NewNFSAuthenticator())
}
