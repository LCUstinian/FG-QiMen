...
// / Authenticate 实现 cred.Authenticator。按顺序尝试每个 cred（先 HTTP
// 再 HTTPS）；首个命中返回 Hit。
func (a *WinRMAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
func init() {
	cred.Register(NewWinRMAuthenticator())
}
