...
// 返回 Hit，否则返回 nil。
func (a *TelnetAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
	for i, c := range creds {
...
func init() {
	cred.Register(NewTelnetAuthenticator())
}
