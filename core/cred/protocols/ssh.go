...
// Authenticate implements cred.Authenticator. Tries each password
// credential in order; returns the first hit or nil.
//
// Authenticate 实现 cred.Authenticator。按顺序尝试每个密码凭据；
// 首个命中返回，否则返回 nil。
func (a *SSHAuthenticator) Authenticate(ctx context.Context, host string, port int, creds []cred.Cred, timeout time.Duration) (*cred.Hit, error) {
	if len(creds) == 0 {
		return nil, nil
	}
...
// 从不打开 session。
func sshTry(ctx context.Context, addr string, c cred.Cred, hkcb ssh.HostKeyCallback, timeout time.Duration) bool {
